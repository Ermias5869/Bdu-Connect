...
import { IoReorderThree, IoSettingsOutline } from "react-icons/io5";
import { Link, useNavigate } from "react-router-dom";
import { IoIosArrowRoundBack } from "react-icons/io";

export default function AllChannalBar() {
    ...
    <aside className="h-screen md:w-60 w-full  bg-white text-black  border-r border-gray-200 flex flex-col transition-all duration-200 overflow-scroll">
      ...
      <div className="border-b border-gray-200 w-full p-5">
        <div className="flex items-center gap-2">
          <span className="font-bold text-2xl text-blue-400">All Channals</span>
        </div>
      </div>

      <div className="overflow-y-auto w-full py-3">
        {Array.isArray(channal) &&
          channal?.map((channal) => (
            <button
              key={channal._id}
            ...
            >
              ...
              <div className=" block text-left min-w-0">
                <div className="font-light truncate text-xs mb-1.5">
                  {channal.name}
                </div>
